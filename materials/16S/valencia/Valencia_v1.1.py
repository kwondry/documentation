...
def combine_taxa(combined_data):
    
    #combining L. jensenii + L. mulieris for purpose of CST assignment
    combined_data['Lactobacillus_jensenii'] = combined_data['Lactobacillus_jensenii'] + combined_data['Lactobacillus_mulieris']
    combined_data = combined_data.drop(['Lactobacillus_mulieris'],axis=1)
    
    #combining together species where genus level typifies the CST 
    combined_data['g_Bifidobacterium'] = combined_data[[col for col in combined_data.columns if col.startswith('Bifidobacterium')]].sum(axis=1)
    combined_data['g_Streptococcus'] = combined_data[[col for col in combined_data.columns if col.startswith('Streptococcus')]].sum(axis=1)
    combined_data['g_Staphylococus'] = combined_data[[col for col in combined_data.columns if col.startswith('Staphylococcus')]].sum(axis=1)
    combined_data['g_Enterococcus'] = combined_data[[col for col in combined_data.columns if col.startswith('Enterococcus')]].sum(axis=1)
    drop_list = [col for col in combined_data.columns if col.startswith('Bifidobacterium')]
    drop_list = drop_list + [col for col in combined_data.columns if col.startswith('Streptococcus')]
    drop_list = drop_list + [col for col in combined_data.columns if col.startswith('Staphylococcus')]
    drop_list = drop_list + [col for col in combined_data.columns if col.startswith('Enterococcus')]
    combined_data = combined_data.drop(drop_list,axis=1)
...
#revising taxa names for purpose of CST assignment
combined_data = combine_taxa(combined_data)
...
sample_data_OG[sample_data_OG['score']==0]['subCST']=='Unassigned'
