...
## Your turn — Exercise 1 {.your-turn}
...
**Don't do this!** One bar per group shows only the mean and throws away the
spread, the shape, and every individual penguin.
...
qs <- penguins |>
  ...
  group_by(species) |>
  summarise(
    q25 = quantile(body_mass_g, 0.25),
    q50 = median(body_mass_g),
    q75 = quantile(body_mass_g, 0.75)
  )

penguins |>
  filter(!is.na(body_mass_g)) |>
  ggplot(aes(x = body_mass_g)) +
  geom_histogram(binwidth = 200, fill = "grey75") +
  geom_vline(data = qs, aes(xintercept = q25), linetype = "dashed") +
  geom_vline(data = qs, aes(xintercept = q50), linetype = "dashed", linewidth = 1) +
  geom_vline(data = qs, aes(xintercept = q75), linetype = "dashed") +
  facet_grid(cols = vars(species))
```

Each dashed line **cuts** the distribution: 25% of penguins fall below the
first, half below the **median**, 75% below the third.
...
A **boxplot** is just those three cuts drawn as a box: the edges are the **25%**
and **75%** lines, the middle line is the **median**. One box per group — easy
to compare.
...
## Your turn — Exercise 2 {.your-turn}
...
## Your turn — Exercise 3 {.your-turn}
