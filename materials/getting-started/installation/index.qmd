...
Install mamba on O2 terminal (the installer is interactive — accept the license, accept the default install location, and enter `yes` when asked to run `conda init`):
...
Snakemake is a package that helps structure and streamline code writing for bioinformatics pipelines, which is particularly useful for complicated pipelines that involve multiple steps and a huge number of files. Snakemake 8 and later moved cluster support out of the core package into separate executor plugins, so submitting jobs to O2's Slurm scheduler needs one installed alongside Snakemake itself. Create your first conda environment named `snakemake_9_27_0` with Snakemake and the Slurm executor plugin installed with the following command.
...
```{python}
#| eval: false
mamba create -n snakemake_9_27_0 -c conda-forge -c bioconda snakemake=9.27.0  snakemake-executor-plugin-slurm  snakemake-executor-plugin-slurm-jobstep
```
...
```{python}
#| eval: false
mamba activate snakemake_9_27_0
```
...
### Use a VS Code tunnel to access an O2 compute node
...
### Setup O2 access with VSCode: Create a passphrase to expedite O2 access
